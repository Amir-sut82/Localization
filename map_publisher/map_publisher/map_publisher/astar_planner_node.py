...
Uses diagonal distance from robot center to corners (Pythagorean theorem)
for proper collision-free path planning.
"""
...
class AStarPlannerService(Node):
    def __init__(self):
        ...
        # Robot footprint parameters (from URDF)
        self.declare_parameter('robot_length', 0.4)   # meters (front to back)
        self.declare_parameter('robot_width', 0.46)   # meters (left to right, wheel separation)
        self.declare_parameter('safety_margin', 1.05)  # extra margin
        ...
        # Calculate robot radius as diagonal from center to corner + safety margin
        # Using Pythagorean theorem: r = sqrt((L/2)^2 + (W/2)^2)
    ...
    def map_callback(self, msg):
        """Store the occupancy grid map and create inflated version"""
        self.map_data = msg
        ...
        # Create inflated map for collision checking
    ...
    def create_inflated_map(self):
        ...
        # Find all obstacle cells (value > 50 means occupied)
        obstacle_coords = np.where(original_map > 50)
        
        # Also treat unknown cells (-1) as obstacles for safety
                    ...
                    if dx*dx + dy*dy <= inflation_cells*inflation_cells:
                        ...
                        if 0 <= ny < height and 0 <= nx < width:
                            if self.inflated_map[ny, nx] == 0:
                                self.inflated_map[ny, nx] = 50  # Mark as semi-obstacle
    ...
    def plan_callback(self, request, response):
        """Service callback - plan path from current pose to goal"""
    ...
    def is_valid(self, x, y):
        """Check if a grid cell is valid using inflated map"""
        width = self.map_data.info.width
        height = self.map_data.info.height
        ...
        # Check inflated map for collision
        cell_value = self.inflated_map[y, x]
        
        # Free: 0, Semi-obstacle: 50, Occupied/Inflated: >50
        if cell_value >= 50:  # Occupied, inflated, or semi-obstacle
            return False
    ...
    def get_neighbors(self, x, y):
        ...
        for dx, dy, cost in directions:
            ...
            if self.is_valid(new_x, new_y):
                # For diagonal moves, also check if we can pass through corners
